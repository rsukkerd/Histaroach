 ...
 * 
 * MixedRevision maintains a base revision, this revision is 
 * not to be modified.
 * 
 * MixedRevision has methods to mix in a file from some other 
 * revision to the base revision, compile and run tests on it. 
 * These methods determine the compilability and test result of 
 * this MixedRevision.
 */
public class MixedRevision {
	...
	/**
	 * mix in a file from other revision to the base revision
	 */
	public void mixIn(String filename, Revision otherRevision) throws IOException {
		int exitValue = otherRevision.checkoutRevision();
		assert (exitValue == 0);
		
		copyFile(filename, repoDir, OTHER_TMP_DIR);
		
		exitValue = baseRevision.checkoutRevision();
		assert (exitValue == 0);
	 ...
	 */
	public void compile() {
		boolean build = baseRevision.build(repository.antBuild);
		boolean buildtest = baseRevision.build(repository.antBuildtest);
	...
	/**
	 * compile and run all tests on the base revision
	 * @modifies this
	 */
	public void compileAndRunAllTests() {
		testResult = baseRevision.run(repository.antJunit);
